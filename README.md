...
The following table explains how each optimized training config differs from that baseline and what change it is expected to make. The measured deltas are from the in-progress full run `runs/experiments/full_20260601_122347/results.jsonl` against the optimized baseline result in the same ledger: GPU mean MAPE `3.891%` and CPU PyTorch fp32 p50 `6.934 ms`. Negative MAPE delta is better; CPU speedup is higher-is-better.
