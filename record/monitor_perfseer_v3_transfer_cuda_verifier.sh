...
job="${PERFSEER_NAUTILUS_JOB:-perfseer-v3-transfer-cuda-verifier}"
log_path="${1:-record/perfseer_v3_transfer_cuda_verifier_monitor.log}"
started_at="$(date +%s)"
...
while true; do
  ...
  elapsed="$((now - started_at))"
  {
    ...
    if [[ -n "$pod" ]]; then
      ...
      kubectl logs "$pod" --namespace "$namespace" --all-containers --tail=160 2>&1 || true
      kubectl exec "$pod" --namespace "$namespace" -c verifier -- /bin/bash -lc \
        'tail -n 100 /workspace/perfseer_v3_transfer_cuda_verifier.log 2>/dev/null || true; find /workspace -maxdepth 2 -type f \( -name "*.log" -o -name "*.json" -o -name "*.pt" \) -printf "%p %s bytes\n" 2>/dev/null | sort' \
        2>&1 || true
    fi
