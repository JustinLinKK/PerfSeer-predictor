#!/usr/bin/env python3
"""Render and strictly verify a digest-only native-A10 Nautilus Job."""
...
IMAGE_RE = re.compile(r"^gitlab-registry\.nrp-nautilus\.io/[a-zA-Z0-9._/-]+@sha256:[0-9a-f]{64}$")
...
def verify_job(value: Mapping[str, Any], *, mode: str, chunk_index: int | None = None) -> None:
    ...
    if len(containers) != 1:
        raise ValueError("A10 Job must contain exactly one container")
    ...
    if resources["requests"] != expected_resources or resources["limits"] != expected_resources:
        raise ValueError("requests/limits differ from the exact A10 NRP policy contract")
...
def render(arguments: argparse.Namespace) -> dict[str, Any]:
    ...
    if arguments.mode == "chunk" and (arguments.chunk_index is None or not 0 <= arguments.chunk_index < 70):
    ...
    result["metadata"]["name"] = f"perfseer-v3-a10-{suffix}"
    ...
    if arguments.mode == "chunk":
        ...
        container["args"].extend(["--chunk-index", str(arguments.chunk_index), "--max-new-accepted", "256"])
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--template", type=Path, default=Path("k8s/a10-labeler-job.yaml"))
    parser.add_argument("--output", type=Path, required=True)
    parser.add_argument("--namespace", required=True)
    parser.add_argument("--image", required=True)
    parser.add_argument("--source-revision", required=True)
    parser.add_argument("--pvc", default="perfseer-v3-a10-labels")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    verify_job(_load(arguments.output), mode=arguments.mode, chunk_index=arguments.chunk_index)
