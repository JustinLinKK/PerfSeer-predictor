#!/usr/bin/env bash
set -u

if [[ $# -lt 2 || $# -gt 3 ]]; then
  echo "usage: $0 NAMESPACE JOB_NAME [LOG_PATH]" >&2
  exit 2
fi

monitor_namespace=$1
monitor_job=$2
monitor_repository=$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)
monitor_log=${3:-"$monitor_repository/record/${monitor_job}-monitor.log"}

if [[ ! $monitor_namespace =~ ^[a-z0-9][-a-z0-9]*[a-z0-9]$ ]]; then
  echo "invalid namespace" >&2
  exit 2
fi
if [[ ! $monitor_job =~ ^[a-z0-9][-a-z0-9]*[a-z0-9]$ ]]; then
  echo "invalid Job name" >&2
  exit 2
fi

mkdir -p "$(dirname "$monitor_log")"
touch "$monitor_log"
chmod 0600 "$monitor_log"
monitor_iteration=0

while true; do
  {
    echo "===== $(date --iso-8601=seconds) iteration=$monitor_iteration ====="
    kubectl get job "$monitor_job" --namespace "$monitor_namespace" -o wide || true
    kubectl get pod --namespace "$monitor_namespace" -l "job-name=$monitor_job" -o wide || true
    monitor_pods=$(kubectl get pod --namespace "$monitor_namespace" -l "job-name=$monitor_job" -o jsonpath='{.items[*].metadata.name}' 2>/dev/null || true)
    for monitor_pod in $monitor_pods; do
      echo "----- describe $monitor_pod -----"
      kubectl describe pod "$monitor_pod" --namespace "$monitor_namespace" || true
      echo "----- logs $monitor_pod -----"
      kubectl logs "$monitor_pod" --namespace "$monitor_namespace" --all-containers=true --tail=250 || true
      echo "----- termination $monitor_pod -----"
      kubectl get pod "$monitor_pod" --namespace "$monitor_namespace" -o jsonpath='{range .status.containerStatuses[*]}name={.name} ready={.ready} restarts={.restartCount} reason={.state.terminated.reason} exitCode={.state.terminated.exitCode}{"\n"}{end}' || true
      echo "----- processes $monitor_pod -----"
      kubectl exec "$monitor_pod" --namespace "$monitor_namespace" -- sh -c 'for process in /proc/[0-9]*; do test -r "$process/comm" || continue; printf "%s " "${process##*/}"; tr "\000" " " < "$process/cmdline" 2>/dev/null; echo; done' || true
      echo "----- persistent logs and outputs $monitor_pod -----"
      kubectl exec "$monitor_pod" --namespace "$monitor_namespace" -- sh -c 'find /workspace -type f \( -name "*.log" -o -name "*.json" -o -name "*.jsonl" -o -name "*.pt" \) -printf "%TY-%Tm-%TdT%TH:%TM:%TS %s %p\n" 2>/dev/null | sort | tail -200' || true
      kubectl exec "$monitor_pod" --namespace "$monitor_namespace" -- sh -c 'find /workspace -type f -name "*.log" -print0 2>/dev/null | xargs -0 -r tail -n 40' || true
    done
    echo "----- recent events -----"
    kubectl get events --namespace "$monitor_namespace" --sort-by=.lastTimestamp | tail -100 || true
  } >>"$monitor_log" 2>&1

  monitor_iteration=$((monitor_iteration + 1))
  if (( monitor_iteration < 5 )); then
    sleep 60
  else
    sleep 1200
  fi
done
